#!/usr/bin/env python3
"""
Optimize images for the Noise From The Basement website.
...
def optimize_image(input_path: Path, *, quality: int, max_width: int) -> Path | None:
    ...
    # Leave animated images alone. Saving without save_all writes only the first
    # frame, which silently destroys the animation. Optimize GIFs with a dedicated
    # tool (e.g. gifsicle -O3) instead.
    if getattr(img, "is_animated", False):
        print(f"  Skipping {input_path.name}: animated, leaving original untouched")
        return None
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Optimize images for the Noise From The Basement website.",
    )
    parser.add_argument(
        "images",
        nargs="+",
