#!/usr/bin/env bash
#
# Compress every supported image under content/ to WebP, rewrite references to
# the new filename across content/ and hugo.toml, and remove the original file.
#
# References are only rewritten when they use the page-bundle "images/<name>"
# form, so external image URLs (e.g. legacy wpengine links) are left untouched.
#
# Safe to re-run: images whose .webp sibling already exists are skipped.
#
# Usage:
#   scripts/optimize_all_images.sh            # convert everything under content/
...
SCAN_DIR="content"
REFERENCE_ROOTS=("content" "hugo.toml")
...
for image in "${IMAGES[@]}"; do
    ...
    if [[ $DRY_RUN -eq 1 ]]; then
        ...
        echo "  would rewrite references: images/$base → images/$webp_base"
    ...
    # Rewrite references to the page-bundle "images/<name>" path across content/
    # and hugo.toml. Filename is passed via env so shell/regex specials in the
    # name are safe; \Q...\E makes perl treat the old name as a literal string.
    # The (?<=images/) lookbehind scopes the match to local page-bundle refs so
    # external URLs sharing the same basename are never rewritten, and the
    # lookahead prevents matching inside a longer filename (e.g. "a.png.bak").
    ref_count=0
    while IFS= read -r -d '' refpath; do
        OLD_NAME="$base" NEW_NAME="$webp_base" perl -i -pe '
            s{(?<=images/)\Q$ENV{OLD_NAME}\E(?![A-Za-z0-9_.\-])}{$ENV{NEW_NAME}}g
        ' "$refpath"
        ref_count=$((ref_count + 1))
    done < <(grep -rlI --null --fixed-strings -- "images/$base" "${REFERENCE_ROOTS[@]}" 2>/dev/null || true)

    rm -- "$image"

    if [[ $ref_count -gt 0 ]]; then
        echo "  updated $ref_count file(s) referencing images/$base"
    else
        echo "  no references to images/$base found"
    fi
