...
\[sourcecode language="csharp" wraplines="false"\] public static Thread StartMethodInNewThread(ThreadStart methodToStart) { // Do common some stuff before starting the thread, such as logging.

// Start the task in a new thread Thread t = new Thread(methodToStart); t.Start();

// Do some stuff after starting the thread, such as more logging.

// If the caller cares you can return the thread. return t; } \[/sourcecode\]

Now anywhere in your code where you need to start a new method in a thread write the following:

\[sourcecode language="csharp" wraplines="false"\] // Example of starting an instantiated object method in // a new thread. FooBar fb = new FooBar(); ThreadStart ts = new ThreadStart(delegate() { fb.Foo(1, 2); }); \_fooThread = ThreadHelper.StartMethodInNewThread(ts);

// Example of a static method. ThreadStart ts = new ThreadStart(delegate() { FooBar.Bar(3, "String D"); }); \_barThread = ThreadHelper.StartMethodInNewThread(ts); \[/sourcecode\]
